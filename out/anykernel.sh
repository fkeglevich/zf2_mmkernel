...
kernel.string=FlareX
do.devicecheck=1
do.initd=0
do.modules=0
do.cleanup=1
device.name1=Z00A
device.name2=ASUS_Z00A
device.name3=Z008
device.name4=ASUS_Z008
device.name5=
...
write_boot() {
  ...
  cat /tmp/anykernel/boot-new2.img /tmp/anykernel/boot_z00a.sig > /tmp/anykernel/boot-new.img;
...
# Modules: Delete all .ko files in /lib/modules and copy our modules into the folder
cd $ramdisk;
rm ./lib/modules/*.ko;
cp -af /tmp/anykernel/modules/*.ko ./lib/modules/;
chmod 0644 ./lib/modules/*.ko;

# Fstab: make it compatible to work with both f2fs or ext4
cp -f /tmp/anykernel/fstab $ramdisk;
cp -f /tmp/anykernel/fstab.mofd_v1 $ramdisk;
